...
export default class PhonebookContactInfo_AutoDialView_ver2{
    ...
    constructor( contact ) {
        ...
        if( contact ) {
            ...
            const lang = BrekekeOperatorConsole.getStaticInstance().getLoggedinLanguage();
            const phonebook = Brekeke.Phonebook.getManager(lang);
            for (const [key, value] of infoEntries) {
                const phonebookItem = phonebook.item.find( (itm) => itm.id === key );

                let title;
                let type;
                if( phonebookItem ){
                    title = phonebookItem.caption;
                    type = phonebookItem.type;
                }
                else{
                    title = null;
                    type = null;
            ...
            const phonebook = Brekeke.Phonebook.getManager( lang );
            for( let i = 0; i < phonebook.item.length; i++ ){
                const itm = phonebook.item[i];
                const onscreen = itm["onscreen"];
                if( onscreen !== true  ){
                    continue;
                }
                const pbContactInfozInfo = PhonebookContactInfozInfoFactory_AutoDialView_ver2.createPhonebookContactInfozInfoTry(itm.id, itm.caption, initialValue, itm.type );
                if (pbContactInfozInfo instanceof PhonebookContactInfozTelInfo_AutoDialView_ver2) {
                    this._TelInfoKeyNameArray.push(pbContactInfozInfo.getInfoKeyName());
                    this._TelInfoTitleArray.push(pbContactInfozInfo.getTitle());
                    if (pbContactInfozInfo.getValue() && pbContactInfozInfo.getValue().length !== 0) {
