...
export default class LegacyButtonRuntimeSubWidget_nextCallButton extends LegacyButtonRuntimeSubWidget  {
    ...
    getRenderJsx() {
        ...
        if( currentCallIndex === callInfoCount - 1 ){
            onClick = (ev) => oc.switchToNewCall();
        }
        else if( currentCallIndex < callInfoCount - 1 ){
            onClick = (ev) => oc.switchCallDown();
        }
        else{
            onClick = undefined;
        }
        return <button title={i18n.t(`legacy_button_description.${subtypeName}`)}  className={clsx("kbc-button kbc-button-fill-parent", (currentCallIndex < callInfoCount - 1) && "kbc-button-danger-flash  kbc-nextCall-button-danger-flash")}
                       style={{
                       ...
                       //onClick={(!callInfoCount || currentCallIndex === callInfoCount - 1) ? undefined : oc.switchCallDown}
                        onClick = { onClick }
        >{iconJsx}</button>
    }
